...
static sss_object_t platform_scp_enc_key_object;
static sss_object_t platform_scp_mac_key_object;
...
static int setup_host(void)
{
    ...
    err = sss_host_key_object_init(&platform_scp_enc_key_object, &platform_scp_keystore);
    if (err) {
        return -4;
    }

    err = sss_host_key_object_allocate_handle(&platform_scp_enc_key_object, PLATFORM_SCP_ENC_KEY_ID,
    ...
    err = sss_host_key_store_set_key(&platform_scp_keystore, &platform_scp_enc_key_object, 
    ...
    err = sss_host_key_object_init(&platform_scp_mac_key_object, &platform_scp_keystore);
    if (err) {
        return -7;
    }

    err = sss_host_key_object_allocate_handle(&platform_scp_mac_key_object, PLATFORM_SCP_MAC_KEY_ID,
                                              kSSS_KeyPart_Default, kSSS_CipherType_AES,
                                              PLATFORM_SCP_KEY_SIZE, kKeyObject_Mode_Transient);
    ...
    err = sss_host_key_store_set_key(&platform_scp_keystore, &platform_scp_mac_key_object, 
...
{
    static NXSCP03_StaticCtx_t scp03_context = {
        .keyVerNo = PLATFORM_SCP_KVN,
        .Enc = &platform_scp_enc_key_object,
        .Mac = &platform_scp_mac_key_object,
    };
    ...
    SE_Connect_Ctx_t connection_context = {
        ...
        .auth.ctx.scp03.pStatic_ctx = &scp03_context,
    };
